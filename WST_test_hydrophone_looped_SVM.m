...
grid on 
subplot(2,2,2)
first = [S{2}.signals{1},S{2}.signals{2},S{2}.signals{3},S{2}.signals{4}]';
second = [S{3}.signals{1},S{2}.signals{3},S{2}.signals{3},S{3}.signals{4},S{3}.signals{5}]';
...
for ii = 1:length(C)
    for i = 1 :k
        ...
        svmParams_looped = templateSVM('KernelFunction', 'RBF', 'BoxConstraint', C(ii), 'KernelScale', 10, 'Standardize', true);
...
title('Support Vector Machine Trained with Hydrophone Data WST','Looped Water Network','FontWeight','bold')
% Additional settings if required
ax = gcf;
exportgraphics(ax,'SVM_looped_hydrophone_WST.png')
%%
...
title('Test ROC Curves of SVM for the Hydrophone Data', 'Looped Water Network','FontWeight','bold')
legend('Location', 'Best', 'FontSize', 8)
grid on 
ax = gcf;
exportgraphics(ax,'SVM_looped_hydrophone_WST_ROCS.png')
%%
