...
accuracy_avg_test_branched = zeros(1,length(leafsize));
accuracy_avg_train_branched = zeros(1,length(leafsize));
accuracy_std_test_branched = zeros(1,length(leafsize));
accuracy_std_train_branched = zeros(1,length(leafsize));
%%
for ii = 1:length(leafsize)
    for i = 1 :k
        ...
        mdl = fitctree(xTrain_cv_branched, yTrain_cv_branched,'MinLeafSize', leafsize(ii));
...
%%
plot(leafsize,accuracy_avg_train_branched,'o-','LineWidth',3);
hold on 
plot(leafsize,accuracy_avg_test_branched,'o-','LineWidth',3);
...
title('Decision Tree Trained with Dynamic Pressure Data WST','Branched Water Network','FontWeight','bold')
% Additional settings if required
ax = gcf;
exportgraphics(ax,'DT_branched_pressure_WST.png')
%%
%%
mdl_final_branched = fitctree(tblTrain_h_branched, yTrain_h_branched,'MinLeafSize', leafsize(r));
%%
...
title('Test ROC Curves of DT for the Dynamic Pressure Data', 'Branched Water Network','FontWeight','bold')
legend('Location', 'Best', 'FontSize', 8)
grid on 
ax = gcf;
exportgraphics(ax,'DT_branched_pressure_WST_ROCS.png')
%%
