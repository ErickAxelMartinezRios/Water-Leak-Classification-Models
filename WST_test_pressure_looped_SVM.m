...
grid on 
title('Dynamic Pressure data WST coefficients for the Looped Water Network')
ax = gca;
exportgraphics(ax,'WST_coefficients_leak_looped_pressure.png')
...
accuracy_avg_test_looped = zeros(1,length(C));
accuracy_avg_train_looped = zeros(1,length(C));
accuracy_std_test_looped = zeros(1,length(C));
accuracy_std_train_looped = zeros(1,length(C));
%%
for ii = 1:length(C)
    for i = 1 :k
        ...
        svmParams_looped = templateSVM('KernelFunction', 'RBF', 'BoxConstraint', C(ii), 'KernelScale', 1, 'Standardize', true);
        mdl = fitcecoc(xTrain_cv_looped, yTrain_cv_looped,'Learners',svmParams_looped);
...
%%
plot(C,accuracy_avg_train_looped,'o-','LineWidth',3);
hold on 
plot(C,accuracy_avg_test_looped,'o-','LineWidth',3);
...
title('Support Vector Machine Trained with Dynamic Pressure Data WST','Looped Water Network','FontWeight','bold')
% Additional settings if required
ax = gcf;
exportgraphics(ax,'SVM_looped_pressure_WST.png')
%%
svmParams_final_looped = templateSVM('KernelFunction', 'RBF', 'BoxConstraint', C(r), 'KernelScale', 1, 'Standardize', true);
mdl_final_looped = fitcecoc(tblTrain_h_looped, yTrain_h_looped,'Learners',svmParams_final_looped);
%%
...
title('Test ROC Curves of SVM for the Pressure Data', 'Looped Water Network','FontWeight','bold')
legend('Location', 'Best', 'FontSize', 8)
grid on 
ax = gcf;
exportgraphics(ax,'SVM_looped_pressure_WST_ROCS.png')
%%
