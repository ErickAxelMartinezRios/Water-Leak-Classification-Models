...
accuracy_avg_test_looped = zeros(1,length(n));
accuracy_avg_train_looped = zeros(1,length(n));
accuracy_std_test_looped = zeros(1,length(n));
accuracy_std_train_looped = zeros(1,length(n));
%%
for ii = 1:length(n)
    for i = 1 :k
        ...
        mdl = fitcknn(xTrain_cv_looped, yTrain_cv_looped,'NumNeighbors', n(ii), 'Standardize', true);
...
%%
plot(n,accuracy_avg_train_looped,'o-','LineWidth',3);
hold on 
plot(n,accuracy_avg_test_looped,'o-','LineWidth',3);
...
legend("Average Training Accuracy","Average Validation Accuracy","N = 4",'Location','best')
title('K-Nearest Neighbors Trained with Dynamic Pressure Data WST','Looped Water Network','FontWeight','bold')
% Additional settings if required
ax = gcf;
exportgraphics(ax,'KNN_looped_pressure_WST.png')
%%
mdl_final_looped = fitcknn(tblTrain_h_looped, yTrain_h_looped,'NumNeighbors', n(r), 'Standardize', true);
%%
...
title('Test ROC Curves of KNN for the Dynamic Pressure Data', 'Looped Water Network','FontWeight','bold')
legend('Location', 'Best', 'FontSize', 7)
grid on 
ax = gcf;
exportgraphics(ax,'KNN_looped_pressure_WST_ROCS.png')
%%
...
titleText = sprintf('Test Results of the KNN for the Dynamic Pressure Data\nfor the Looped Water Network');
cm.Title = titleText;
