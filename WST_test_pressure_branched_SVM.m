%Load Dataset 
load("Data_Dynamic_Pressure_Water_Leak.mat","s")
...
ylabel('Dynamic Pressure (Pa)', 'FontWeight', 'bold');
...
%%
plot(scat_features(1,:),'-o','LineWidth',3)
hold on 
plot(scat_features(241+80,:),'-o','LineWidth',3)
hold on 
plot(scat_features(483,:),'-o','LineWidth',3)
hold on 
plot(scat_features(729+79,:),'-o','LineWidth',3)
hold on 
plot(scat_features(962+6,:),'-o','LineWidth',3)
...
grid on 
title('Dynamic Pressure data WST coefficients for the Branched Water Network')
ax = gca;
exportgraphics(ax,'WST_coefficients_leak_branched_pressure.png')
...
C = [0.001,0.1,1,10,50,100,250,500,750,1000];
accuracies_cv_test_branched = zeros(1, k);
accuracies_cv_train_branched = zeros(1,k);
accuracy_avg_test_branched = zeros(1,length(C));
accuracy_avg_train_branched = zeros(1,length(C));
accuracy_std_test_branched = zeros(1,length(C));
accuracy_std_train_branched = zeros(1,length(C));
%%
for ii = 1:length(C)
    for i = 1 :k
        ...
        svmParams_branched = templateSVM('KernelFunction', 'RBF', 'BoxConstraint', C(ii), 'KernelScale', 1, 'Standardize', true);
        mdl = fitcecoc(xTrain_cv_branched, yTrain_cv_branched,'Learners',svmParams_branched);
...
%%
plot(C,accuracy_avg_train_branched,'o-','LineWidth',3);
hold on 
plot(C,accuracy_avg_test_branched,'o-','LineWidth',3);
[p,r] = max(accuracy_avg_test_branched)
xline(C(r),'--','LineWidth',2);
grid on 
xlabel('C parameter','FontWeight','bold')
ylabel('Average Accuracy %','FontWeight','bold')
legend("Average Training Accuracy","Average Validation Accuracy","C = 1000",'Location','southeast')
title('Support Vector Machine Trained with Dynamic Pressure Data WST','Branched Water Network','FontWeight','bold')
% Additional settings if required
ax = gcf;
exportgraphics(ax,'SVM_branched_pressure_WST.png')
%%
%%
svmParams_final_branched = templateSVM('KernelFunction', 'RBF', 'BoxConstraint', C(r), 'KernelScale', 1, 'Standardize', true);
mdl_final_branched = fitcecoc(tblTrain_h_branched, yTrain_h_branched,'Learners',svmParams_final_branched);
%%
...
title('Test ROC Curves of SVM for the Dynamic Pressure Data', 'Branched Water Network','FontWeight','bold')
legend('Location', 'Best', 'FontSize', 8)
grid on 
ax = gcf;
exportgraphics(ax,'SVM_branched_pressure_WST_ROCS.png')
%%
...
titleText = sprintf('Test Results of the SVM for the Dynamic Pressure Data\nfor the Branched Water Network');
cm.Title = titleText;